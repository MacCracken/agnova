# Agnova — Claude Code Instructions

## Project Identity

**Agnova** (Latin: nova — new) — OS installer for AGNOS
...
End users (installation). Agnova is the guided installer that partitions disks, deploys the base system, and bootstraps AGNOS on new hardware.
...
- Disk operations must be idempotent where possible — interrupted installs should be resumable
- Partitioning must be explicit and confirmed — no silent destructive operations
- Installation state must be checkpointed — crash recovery at every stage
...
- Do not perform destructive disk operations without explicit user confirmation
- Do not assume hardware — detect and validate before partitioning
